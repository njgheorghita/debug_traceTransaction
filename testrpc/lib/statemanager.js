...
var createRpcVm = require('ethereumjs-vm/lib/hooked').fromWeb3Provider
var EthQuery = require('../../eth-query/index.js')
const Readable = require('stream').Readable
const ConcatStream = require('concat-stream')
const materializeBlock = require('../../eth-tx-summary/materialize-blocks.js')
...
///////////////////////////////////////
StateManager.prototype.generateTraceTransaction = function(provider, txHash, cb) {
  var previousBlockNumber
  console.log("FUCK ME")
  this.blockchain.getTransactionReceipt(txHash, (err, res) => previousBlockNumber = utils.bufferToInt(res.block.header.number) -1)
  // VM
  var fuck
  this.blockchain.getBlock(2, (err, res) => fuck = res)
  fuck.header.raw.forEach((elem)=>console.log(utils.bufferToHex(elem)))

  // here you have access to this.blockchain.transactions
  // grab prepatory txs from ^
  // run prepatory txs on v
  var david = new ForkedBlockchain({fork: true, fork_block_number: previousBlockNumber});
  eval(pry.it)
  vm = createRpcVm(provider, previousBlockNumber, {
...
function createVmTraceStream(provider, tx_hash) {
  var self = this;
  ...
  function prepareVM(cb) {
    eval(pry.it)
    query.getTransactionByHash(tx_hash, function(err, _txData){
      console.log(err)
      if (err) return cb(err)
      if (!_txData) cb(new Error('No transaction found...'))
      txData = _txData
      traceStream.push({
        type: 'tx',
        data: txData,
      ...
      //   var backingStateBlockNumber = utils.intToHex(parseInt(blockData.number, 16)-1)
  ...
  /*
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  need to find the correct provider to pass to generateTxSummary
  which is self? testrpc.provider? web3.currentProvider?
  

  or we run the logic from createVmTraceStream here, internally

  if i want to replay a transaction from the state manager 
  can I use:
  ...
  */
  ...
  // var totalGas = 0; // better way to init this?
  // self.blockchain.getTransactionReceipt(tx_hash, function(err, receipt) {
  //   if (err) return callback(err);
  //   receipt = receipt.toJSON();
  ...
  // })
  // gas: totalGas,
  // returnValue: what is this?,
  // structLogs: [ {this.blockchain.getTransactionLogs(tx_hash, callback)} ]
  this.blockchain.getTransactionLogs(provider, tx_hash, function(err, logs) {
    if (err) return callback(err);
    console.log(logs)
  });
}
