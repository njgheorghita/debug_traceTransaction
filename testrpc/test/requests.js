...
/////////////////////////////
  describe("debug_traceTransaction", function() {
    it.only("should return debug info for a transaction", function(done) {
      /////// send a transaction
      web3.eth.sendTransaction({
        from: accounts[1],
        data: contract.binary,
        gas: 3141594
      }, (err, tx_hash) => console.log(tx_hash))
      web3.eth.sendTransaction({
        from: accounts[3],
        data: contract.binary,
        gas: 3141593
      ...
      /////// trace the transaction
        ...
        web3.eth.traceTransaction(tx_hash, function(err, result) {
          ///// web3.debug.traceTransaction
          if (err) return done(err);
